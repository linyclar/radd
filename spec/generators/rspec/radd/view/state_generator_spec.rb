...
require "spec_helper"
require "generators/rspec/radd/view/state/state_generator"

RSpec.describe Rspec::Radd::View::Generators::StateGenerator, type: :generator do
  ...
  describe "#copy_state" do
    ...
    context "bin/rails g radd:state untreated" do
      it_behaves_like("ファイルが生成される", "untreated", "spec/view/state/untreated_view_state.rb")
    end

    context "bin/rails g radd:state accounting_feature/untreated" do
      it_behaves_like("ファイルが生成される", "accounting_feature/untreated",
                      "spec/view/state/accounting_feature/untreated_view_state.rb")
    end
  end
end
